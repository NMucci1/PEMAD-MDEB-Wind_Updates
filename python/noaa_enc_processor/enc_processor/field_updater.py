    ...
    try:
        ...
        for item_id, csv_path in map.items():
            ...
            try:
                ...
                # Get the layer definition and convert it to a python dictionary
                layer_definition_dict = dict(layer_to_update.properties)
                
                updated_fields_list = []

                # Loop through the fields from the layer definition dictionary
                for field in layer_definition_dict['fields']:
                    field_name = field['name']
                    
                    if field_name in field_lookup:
                        # Update the alias and description using the field lookup
                ...
                update_dict = {"fields": updated_fields_list}
                
                # Apply the changes using the new dictionary
